...
document.querySelectorAll(".coeur").forEach(function (coeurImg) {
    coeurImg.addEventListener("click", function () {
        // Récupérer l'ID de la région à partir du parent ou d'un attribut data
        const regionId = this.closest(".destination-region").dataset.regionId;

        // Vérifier l'état actuel du like
        const isLiked = this.src.includes("coeur-rouge.png");

        // Alterner l'image
        this.src = isLiked ? "static/img/coeur-blanc.png" : "static/img/coeur-rouge.png";

        // Envoyer la requête au serveur pour enregistrer le like/dé-like
        fetch(`/like?region_id=${regionId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({ liked: !isLiked }), // Envoyer l'état actuel
        })
            .then((response) => response.json())
            .then((data) => {
                console.log(`Like mis à jour pour la région ${regionId}:`, data);
            })
